...
movies = pd.read_csv("exportedData/moviesExpUncut.csv", usecols=["movieId","title", "tmdbId", "genres_list"])
...
moviesExt = moviesExt.drop("vote_average", axis=1)
moviesExt = moviesExt.drop("vote_count", axis=1)
moviesExt = moviesExt.drop("id", axis=1)
...
result = pd.DataFrame()
ratings = pd.read_csv("exportedData/ratingsExpUncut.csv").copy()
ratingsA = ratings.groupby("movieId").mean().rating
moviesExt = moviesExt.merge(ratingsA, on="movieId", how="inner")

moviesExt[moviesExt["title"] == "Dark Knight, The (2008)"]["rating"].item()
...
a = recommend("Dark Knight, The (2008)")
temp = pd.merge(a, moviesExt, on="title", how="inner")
tempG = temp.drop(["cast","crew","keywords","overview","movieId","rating","tmdbId","director"], axis=1)
tempR = temp.drop(["cast","crew","keywords","overview","movieId","genres_list","tmdbId","director"], axis=1)
tempG




# Remove Recommendations of movies already rated by user x

ratingsA["rating"]

b = moviesExt[moviesExt["title"] == "Dark Knight, The (2008)"]
"""
user = 52

# find rated movieIds
rated = ratings[ratings["userId"] == 52]
rated = set(rated["movieId"])

res = recommend('Dark Knight, The (2008)')["title"]
...
for i in range(len(res)):
    for j in range(len(moviesList)):
        if res.iloc[i] == moviesList["title"][j]:
            print(moviesList["movieId"][j])
            temp[count] = moviesList["movieId"][j]
            count += 1
"""
